...
class App(tk.Tk):
    def __init__(self):
        ...
        ttk.Label(row, text="Drag a rectangle on the preview to set the crop.").pack(side="left", padx=14)
        ...
        for label, variable, width in [("Sample every (s)", self.interval, 5), ("Change threshold", self.threshold, 6),
                                      ("Start (s)", self.start_time, 6), ("End (s; blank = all)", self.end_time, 6)]:
            ttk.Label(options, text=label).pack(side="left", padx=(0, 5))
            ttk.Entry(options, textvariable=variable, width=width).pack(side="left", padx=(0, 16))
        self.action(setup, "Extract score lines", self.extract_video).pack(anchor="e")
        ttk.Checkbutton(setup, text="Remove overlapping lines when pages scroll", variable=self.remove_overlap).pack(anchor="w")
